...
typedef bool (*gl_setelement_threshold_fn)(const int *elt,
		const int *threshold);
...
struct gl_oset_implementation {
	...
	bool (*search)(gl_oset_t set, const int elt);
	bool (*search_atleast)(gl_oset_t set,
			gl_setelement_threshold_fn threshold_fn, const int *threshold,
			const int *eltp);
	int (*nx_add)(gl_oset_t set, const int elt);
	bool (*value_at)(gl_oset_t set, size_t position, int* value);
	bool (*remove_elt)(gl_oset_t set, int elt);
	bool (*remove_at)(gl_oset_t set, size_t idx, int* value);
	void (*oset_free)(gl_oset_t set);
	/* gl_oset_iterator_t functions.  */
	gl_oset_iterator_t (*iterator)(gl_oset_t set);
	bool (*iterator_next)(gl_oset_iterator_t *iterator, int *eltp);
...
gl_oset_nx_create_empty(gl_oset_implementation_t implementation,
		gl_setelement_compar_fn compar_fn, gl_setelement_dispose_fn dispose_fn) {
...
GL_OSET_INLINE bool gl_oset_search(gl_oset_t set, int elt) {
	return ((const struct gl_oset_impl_base *) set)->vtable->search(set, elt);
}

GL_OSET_INLINE bool gl_oset_search_atleast(gl_oset_t set,
		gl_setelement_threshold_fn threshold_fn, int *threshold,
		int *eltp) {
	return ((const struct gl_oset_impl_base *) set)->vtable->search_atleast(set,
			threshold_fn, threshold, eltp);
}
...
#endif
*/
gl_oset_nx_add(gl_oset_t set, int elt) {
	return ((const struct gl_oset_impl_base *) set)->vtable->nx_add(set, elt);
}

GL_OSET_INLINE bool gl_oset_value_at(gl_oset_t set, size_t position, int* value) {
	return ((const struct gl_oset_impl_base *) set)->vtable->value_at(set,
			position, value);
}

GL_OSET_INLINE bool gl_oset_remove(gl_oset_t set, int elt) {
	return ((const struct gl_oset_impl_base *) set)->vtable->remove_elt(set,
			elt);
}

GL_OSET_INLINE bool gl_oset_remove_at(gl_oset_t set, size_t idx, int* value) {
	return ((const struct gl_oset_impl_base *) set)->vtable->remove_at(set,
			idx, value);
}

GL_OSET_INLINE void gl_oset_free(gl_oset_t set) {
...
GL_OSET_INLINE bool gl_oset_iterator_next(gl_oset_iterator_t *iterator, int *eltp) {
	return iterator->vtable->iterator_next(iterator, eltp);
}

GL_OSET_INLINE void gl_oset_iterator_free(gl_oset_iterator_t *iterator) {
