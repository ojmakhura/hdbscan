...
#include "gnulib/xsize.h"

/* -------------------------- gl_oset_t Data Type -------------------------- */

static gl_oset_t gl_array_nx_create_empty(
		gl_oset_implementation_t implementation,
		gl_setelement_compar_fn compar_fn, gl_setelement_dispose_fn dispose_fn) {
	struct gl_oset_impl *set = (struct gl_oset_impl *) malloc(
			sizeof(struct gl_oset_impl));
...
static size_t gl_array_size(gl_oset_t set) {
	return set->count;
}

static size_t gl_array_indexof(gl_oset_t set, const int elt) {
...
static bool gl_array_search(gl_oset_t set, const int elt) {
	return gl_array_indexof(set, elt) != (size_t) (-1);
}

static bool gl_array_search_atleast(gl_oset_t set,
...
static int grow(gl_oset_t set) {
	...
	memory_size = xtimes(new_allocated, sizeof(int));
	if (size_overflow_p(memory_size))
		/* Overflow, would lead to out of memory.  */
		return -1;
	memory = (int*) realloc(set->elements, memory_size);
	if (memory == NULL)
		/* Out of memory.  */
		return -1;
	set->elements = memory;
...
static int gl_array_nx_add_at(gl_oset_t set, size_t position, int elt) {
	size_t count = set->count;
	int *elements;
	size_t i;
...
static bool gl_array_value_at(gl_oset_t set, size_t position, int* value){

	/*int* elements = set->elements;
	*value = elements[position];
	return true;*/

	if(position >= 0 && position < set->allocated){
		int *elements = set->elements;
		*value = elements[position];
		return true;
	} else {
...
/* Remove the element at the given position,
 0 <= position < gl_oset_size (set).  */
static bool gl_array_remove_at(gl_oset_t set, size_t position, int* value) {
	size_t count = set->count;
	int *elements;
	size_t i;
...
static int gl_array_nx_add(gl_oset_t set, const int elt) {
	size_t count = set->count;
	size_t low = 0;
...
static bool gl_array_remove(gl_oset_t set, int elt) {
	size_t index = gl_array_indexof(set, elt);
	if (index != (size_t) (-1)) {

		gl_array_remove_at(set, index, &elt);
		return true;
	} else
		return false;
}

static void gl_array_free(gl_oset_t set) {
...
static gl_oset_iterator_t gl_array_iterator(gl_oset_t set) {
...
static bool gl_array_iterator_next(gl_oset_iterator_t *iterator, int *eltp) {
	...
	if (iterator->count != set->count) {
		...
		iterator->count--;
		iterator->p = iterator->p - 1;
		iterator->q = iterator->q - 1;
	}
...
static void gl_array_iterator_free(gl_oset_iterator_t *iterator) {
}
